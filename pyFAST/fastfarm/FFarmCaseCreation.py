...
class FFarmCaseCreation:
    ...
    def lowResDomain(self,dx_des,Vhub,Cmeander=1.9):
        ...
        xdist = max(self.x)+12.0*self.D-self.X0_Low ## Change this if you want your x-domain to be shorter

        self.nX_Low = round(xdist/self.dX_Low)+1
        self.nY_Low = round(self.Width/self.dY_Low)
        self.nZ_Low = round(self.Zdist_High/self.dZ_Low)+1
        
        Vbar = 0.8*Vhub
        self.NumPlanes = xdist/(self.dT_Low*Vbar)
        self.dr = np.int(self.cmax)
        self.NumRadii = np.ceil(3.*self.D/(2.*self.dr)+1)
...
def WriteFFarmFile(fileIn,fileOut,params,NewFile=True):
    ...
    WrDisDT = params.dT_Low#np.float(np.int(2/params.dT_Low)*params.dT_Low)
    ...
    if NewFile == True:
        ...
        with open(fileOut, 'w') as f:
            ...
            f.write('60.0\tTMax               Total run time (s) [>=0.0]\n')
            f.write('False\tUseSC              Use a super controller? (flag)\n')
            f.write('3\tMod_AmbWind        Ambient wind model (-) (switch) {1: high-fidelity precursor in VTK format, 2: InflowWind module}\n')
            ...
            f.write('{:.1f}\tDT_Low                 Time step for low -resolution wind data input files; will be used as the global FAST.Farm time step (s) [>0.0]\n'.format(params.dT_Low))
            ...
            f.write('{:.3f}\t\tOutDisWindZ        Z coordinates of XY planes for output of disturbed wind data across the low-resolution domain (m) [1 to NOutDisWindXY] [unused for NOutDisWindXY=0]\n'.format(params.HubHt))
            ...
            f.write('DEFAULT\t\tWrDisDT            Time step for disturbed wind visualization output (s) [>0.0] or DEFAULT [DEFAULT=DT] [unused for WrDisWind=False and NOutDisWindXY=NOutDisWindYZ=NOutDisWindXZ=0]\n'.format(WrDisDT))
            f.write('--- OUTPUT ---\n')
            ...
            f.write('0.0\t\tTStart             Time to begin tabular output (s) [>=0.0]\n')
            ...
            f.write('5\t\tNOutDist           Number of downstream distances for wake output for an individual rotor (-) [0 to 9 ]\n')
            f.write('738.0, 984.0, 1230.0, 1476.0, 1722.0          OutDist            List of downstream distances for wake output for an individual rotor (m) [1 to NOutDist ] [unused for NOutDist =0]\n')
            ...
            f.write('{:.3f}\t\tWindVelZ           List of coordinates in the Z direction for wind output (m) [1 to NWindVel] [unused for NWindVel=0]\n'.format(params.HubHt))
            ...
            f.write('"TIAmbT1"\n')
            f.write('"WkAxsXT1D1   , WkAxsXT1D2   , WkAxsXT1D3   , WkAxsXT1D4   , WkAxsXT1D5"\n')
            f.write('"WkAxsYT1D1   , WkAxsYT1D2   , WkAxsYT1D3   , WkAxsYT1D4   , WkAxsYT1D5"\n')
            f.write('"WkAxsZT1D1   , WkAxsZT1D2   , WkAxsZT1D3   , WkAxsZT1D4   , WkAxsZT1D5"\n')
            f.write('"WkPosXT1D1   , WkPosXT1D2   , WkPosXT1D3   , WkPosXT1D4   , WkPosXT1D5"\n')
            f.write('"WkPosYT1D1   , WkPosYT1D2   , WkPosYT1D3   , WkPosYT1D4   , WkPosYT1D5"\n')
            f.write('"WkPosZT1D1   , WkPosZT1D2   , WkPosZT1D3   , WkPosZT1D4   , WkPosZT1D5"\n')
        ...
        ModVars = ['DT','DT_High','NX_Low','NY_Low','NZ_Low','X0_Low','Y0_Low','Z0_Low','dX_Low','dY_Low','dZ_Low','NX_High','NY_High','NZ_High','dr','NumRadii','NumPlanes']
        wt=0
...
def writeDynfiles(rootname,yaw,binvals,nTurbs,NewFile=False):
    """ Write a ElastoDyn, ServoDyn, and HydroDyn input files, 

    """
    import random
    #print('writing new HydroDyn file')
    HydroIn = rootname+'HydroDyn.dat'
    HydroOut = rootname+'HydroDyn_mod.dat'
    #WaveSeed1 = random.randint(-2147483648,2147483647)
    #WaveSeed2 = random.randint(-2147483648,2147483647)
    WvHiCOffD=2.0*np.pi/binvals['WaveTp']  ## =WvLowCOffS
    with open(HydroOut, 'w+') as new_file:
        with open(HydroIn) as old_file:
            for line in old_file.readlines():
                newline = line
                #for tmpVar in ['WaveSeed(1)']:
                #    if tmpVar in line:
                #        newline = str('{:.3f}\t!!Orig is:  '.format(WaveSeed1))+line
                #for tmpVar in ['WaveSeed(2)']:
                #    if tmpVar in line:
                #        newline = str('{:.3f}\t!!Orig is:  '.format(WaveSeed2))+line
                for tmpVar in ['WaveHs']:
                    if tmpVar in line:
                        newline = str('{:.3f}\t!!Orig is:  '.format(binvals['WaveHs']))+line
                for tmpVar in ['WaveTp']:
                    if tmpVar in line:
                        newline = str('{:.3f}\t!!Orig is:  '.format(binvals['WaveTp']))+line
                for tmpVar in ['WvHiCOffD']:
                    if tmpVar in line:
                        newline = str('{:.3f}\t!!Orig is:  '.format(WvHiCOffD))+line
                for tmpVar in ['WvLowCOffS']:
                    if tmpVar in line:
                        newline = str('{:.3f}\t!!Orig is:  '.format(WvHiCOffD))+line
                new_file.write(newline)
    ...
    for turb in range(nTurbs):
        ElastIn = rootname+'ElastoDyn.T'+str(turb)+'.dat'
        ElastOut = rootname+'ElastoDyn.T'+str(turb)+'_mod.dat'
        with open(ElastOut, 'w+') as new_file:
            with open(ElastIn) as old_file:
                for line in old_file.readlines():
                    newline = line
                    for tmpVar in ['NacYaw']:
                        if tmpVar in line:
                            newline = str('{:.3f}\t!!Orig is:  '.format(yaw[turb]))+line
                    for tmpVar in ['RotSpeedx']:
                        if tmpVar in line:
                            newline = str('{:.3f}\t!!Orig is:  '.format(binvals['RotSpeed']))+line
                    for tmpVar in ['BlPitch']:
                        if tmpVar in line:
                            newline = str('{:.3f}\t!!Orig is:  '.format(binvals['BlPitch']))+line
                    new_file.write(newline)
                    
        ServIn = rootname+'ServoDyn.T'+str(turb)+'.dat'
        ServOut = rootname+'ServoDyn.T'+str(turb)+'_mod.dat'
        with open(ServOut, 'w+') as new_file:
            with open(ServIn) as old_file:
                for line in old_file.readlines():
                    newline = line
                    for tmpVar in ['YawNeut']:
                        if tmpVar in line:
                            newline = str('{:.3f}\t!!Orig is:  '.format(yaw[turb]))+line
                    new_file.write(newline)